...
it('bounds actual PNG dimensions, reserves image tokens, and routes visual A3 and A5 through Sol without exposing image bytes in operation metadata',async()=>{
 ...
 expect(calls.map(c=>c.model)).toEqual(['gpt-5.6-sol','gpt-5.6-sol']);expect(JSON.stringify(calls)).not.toContain(png.toString('base64'));expect(calls[0].images).toBeDefined();expect(money(calls[0].max)).toBeGreaterThan(money('0.02048'));
