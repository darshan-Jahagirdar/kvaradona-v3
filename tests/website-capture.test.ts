...
const transport=vi.hoisted(()=>({calls:[] as string[],crowded:false,broken:false,active:0,maxActive:0,slow:false,asyncSlow:false}));
vi.mock('../src/capture/fetch',()=>({robotsAllows:()=>true,publicUrl:async()=>{},safeRead:async(url:string)=>{
 ...
 await new Promise(resolve=>setTimeout(resolve,transport.slow&&url.endsWith('.js')?27000:transport.asyncSlow&&url.endsWith('/late.css')?17000:20));transport.active--;
...
beforeEach(()=>{transport.calls=[];transport.crowded=false;transport.broken=false;transport.active=0;transport.maxActive=0;transport.slow=false;transport.asyncSlow=false;});
...
it('preserves critical loading under an image/font flood and reuses both viewport responses',async()=>{
 ...
 expect(transport.calls.filter(u=>u.endsWith('.png'))).toHaveLength(12);expect(transport.calls.filter(u=>u.endsWith('.woff2'))).toHaveLength(8);
 expect(transport.maxActive).toBeLessThanOrEqual(6);expect(capture.requests).toBe(transport.calls.length);expect(capture.requests).toBeLessThanOrEqual(160);expect(capture.bytes).toBeLessThanOrEqual(25000000);
...
it('waits beyond the old navigation cutoff for critical content and reuses it on desktop',async()=>{
 transport.crowded=true;transport.slow=true;
 const {capture}=await captureWebsiteEvidence('https://fixture.invalid/','fixture.invalid');
 expect(capture.complete).toBe(true);expect(capture.elapsedMs).toBeGreaterThan(25000);
 expect(transport.calls.filter(u=>u.endsWith('/critical.js'))).toHaveLength(1);
 for(const viewport of ['mobile','desktop'])expect(capture.facts.some(f=>f.viewport===viewport&&f.text.includes('Critical script loaded'))).toBe(true);
},45000);

it('waits for a late critical stylesheet after DOMContentLoaded instead of sampling at fifteen seconds',async()=>{
 transport.crowded=true;transport.asyncSlow=true;
 const {capture}=await captureWebsiteEvidence('https://fixture.invalid/','fixture.invalid');
 expect(capture.complete).toBe(true);expect(capture.renderStable).toBe(true);
 expect(capture.elapsedMs).toBeGreaterThan(17000);
 expect(transport.calls.filter(u=>u.endsWith('/late.css'))).toHaveLength(1);
},35000);
