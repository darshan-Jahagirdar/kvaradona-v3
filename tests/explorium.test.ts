...
import {exploriumFilters,exploriumPilotTopic,exploriumIcp} from '../src/domain/explorium-icp';
...
it('reserves 10 page credits, uses actual credit reports and halts denial or uncertain usage without a second provider call',async()=>{
 process.env.EXPLORIUM_API_KEY='fixture-key';let calls=0;
 const operations={run:async(_key:any,_provider:any,_body:any,_usd:any,units:number,_schema:any,dispatch:any)=>{expect(units).toBe(10);return (await dispatch()).response;}} as any;
 ...
 await expect(callExplorium(operations,'test','businesses',{},10,(async()=>new Response(JSON.stringify({data:[]}))) as typeof fetch)).rejects.toThrow('explorium_credit_usage_unknown');
 await expect(callExplorium(operations,'test','businesses',{},10,(async()=>new Response(JSON.stringify({credit_usage:{total_credits:11}}))) as typeof fetch)).rejects.toThrow('explorium_credit_reservation_exceeded');
});
