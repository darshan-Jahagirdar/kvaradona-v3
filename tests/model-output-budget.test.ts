...
it('reserves the enforced output ceiling and refuses limits above the gateway maximum before dispatch',async()=>{
 ...
 const operations={async run(_key:string,_provider:string,params:{max_output_tokens:number},max:string){calls.push({max,tokens:params.max_output_tokens});return {status:'completed',model:'gpt-5.6-terra',id:'fixture',text:'{"answer":"fixture"}'};}} as unknown as OperationGateway;
 ...
 expect(calls.map(c=>c.tokens)).toEqual([2048,1024]);expect(money(calls[0].max)-money(calls[1].max)).toBe(money('0.012288'));
