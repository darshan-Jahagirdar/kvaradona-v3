...
it('holds obsolete company-only discovery before any provider call or ingestion',async()=>{
 ...
 await expect(runStage({rpc},{id:randomUUID(),organization_id:randomUUID(),campaign_id:randomUUID(),opportunity_id:null,stage:'S02',business_key:'fixture',input_hash:'h',input_version:7,schema_version:'1',prompt_version:'12',attempt_token:randomUUID(),attempts:1,payload:{groups:[group],groupIndex:0,maxResearch:3}},{companySearch:async()=>({candidates:records,providerResult:{httpStatus:200,returned:5,invalid:0,requestedPage:1,filters:{} as any,intent:'unknown'}}),search,ai:{generate:async()=>{throw Error('unexpected_model');}},fetchEvidence:async()=>{throw Error('unexpected_fetch');},relationship:async()=> 'unknown',contact:async()=>{throw Error('unexpected_contact');}})).rejects.toThrow('apollo_intent_access_unverified');
 expect(search).not.toHaveBeenCalled();expect(rpc).not.toHaveBeenCalled();
});
