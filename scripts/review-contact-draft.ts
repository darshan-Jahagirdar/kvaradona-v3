...
import {draftResearch,crmReviewProblems} from '../src/domain/crm-specialist';
...
if(p.draftReview&&!reviewProblems(p.draftReview,draftResearch(p),p,JSON.stringify(p.draft)).length){console.log('Exact recipient-bound draft is already checked.');process.exit(0);}
