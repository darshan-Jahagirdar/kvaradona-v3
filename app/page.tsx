...
export default async function Inbox(){
 ...
 const [opps,jobs,status,membership,observations]=await Promise.all([
 ...
 {(opps.error||jobs.error||status.error||observations.error)&&<p role="alert" className="error">Some records could not be loaded. The displayed list may be incomplete.</p>}
 ...
 {items.map(item=>item.parsed.success?<ReviewCard key={`${item.id}:${item.revision}`} id={item.id} revision={item.revision} packet={item.parsed.data} discoveryObservations={(observations.data??[]).filter(o=>o.opportunity_id===item.id)} websiteChecked={Boolean(item.parsed.data.websiteSupplement&&!websiteReviewProblems(item.parsed.data).length)} crmChecked={Boolean(item.parsed.data.crmSupplement&&!crmReviewProblems(item.parsed.data).length)}/>:<section className="card" key={item.id}><h2>Record needs investigation</h2><p>The saved packet does not match the current schema. No approval is available.</p></section>)}
