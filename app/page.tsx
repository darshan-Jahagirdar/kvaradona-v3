import {packetReadiness,recoveryPlan} from '../src/domain/recovery';
...
export default async function Inbox({searchParams}:{searchParams:Promise<{section?:string;filter?:string}>}){
 ...
 const [opps,status,membership,observations,quality,companyObservations]=await Promise.all([
  ...
  client.from('company_discovery_observations').select('id,opportunity_id,source,external_id,observed_at,changed').order('observed_at',{ascending:false}).limit(1000),
 ]);
 ...
 const metrics=status.data as {spent_usd?:string;reserved_usd?:string;limit_usd?:string;dollar_limits_enabled?:boolean}|null;
 ...
 function card(item:ReviewItem){const p=item.packet,placement=reviewPlacement(p),label=labels.get(item.id);return <div id={`opportunity-${item.id}`} key={`${item.id}:${item.revision}:${item.packet_hash}`}><ReviewCard id={item.id} revision={item.revision} packet={p} recovery={recoveryPlan(p)} coverage={packetReadiness(p)} needLabel={placement.need} readinessLabel={placement.readiness} writingCurrent={writingReviewCurrent(p)} discoveryObservations={[...(observations.data??[]),...(companyObservations.data??[])].filter(o=>o.opportunity_id===item.id)} websiteChecked={Boolean(p.websiteSupplement&&!websiteReviewProblems(p).length)} crmChecked={Boolean(p.crmSupplement&&!crmReviewProblems(p).length)}/><QualityReview id={item.id} revision={item.revision} packetHash={item.packet_hash} latest={label?{labels:label.labels,current:label.packet_hash===item.packet_hash}:undefined}/></div>;}
