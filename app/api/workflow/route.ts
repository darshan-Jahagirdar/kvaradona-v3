...
export async function POST(req:NextRequest){
 ...
 if(error){const messages:Record<string,string>={apollo_intent_access_unverified:intentSetup.reason,budget_paused:'Budget paused. Existing spending and unresolved reservations count toward the $2 cap.',live_disabled:'Live execution is paused. No work was launched.',provider_unverified:'A required provider is unavailable. No work was launched.',membership_required:'Organization access is required.',free_quota_unverified_or_exhausted:'The authorized provider trial allowance is exhausted or unavailable.',intent_search_exhausted:'This intent search has no remaining page. Review the current results before changing its scope.',explorium_trial_verification_expired:'Explorium trial verification has expired. Verify remaining free access before continuing.',explorium_credit_accounting_hold:'Explorium credit accounting needs review before another run.'};return NextResponse.json({error:messages[error.message]??'Workflow could not be queued. Try again using the same request.'},{status:409});}
