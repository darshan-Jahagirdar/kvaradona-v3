...
export async function POST(req:NextRequest){
 ...
 if(error){const messages:Record<string,string>={budget_paused:'Budget paused. Existing spending and unresolved reservations count toward the $2 cap.',live_disabled:'Live execution is paused. No work was launched.',provider_unverified:'A required provider is unavailable. No work was launched.',membership_required:'Organization access is required.',free_quota_unverified_or_exhausted:'The authorized Apollo allowance is exhausted or unavailable.'};return NextResponse.json({error:messages[error.message]??'Workflow could not be queued. Try again using the same request.'},{status:409});}
...
export async function GET(){
 ...
 return NextResponse.json({run,companyCount:companyKeys.size,worker:worker.data,budget:budget.data,jobs:jobs.data,opportunities:allOpportunities.map(o=>({id:o.id,state:o.state,name:o.packet.research?.company??o.packet.candidate?.procurementNotice?.buyer??o.packet.candidate?.providerCompany?.name??o.packet.candidate?.providerRecord?.company??o.packet.candidate?.title??'Source being checked'}))},{headers:{'Cache-Control':'no-store'}});
}
