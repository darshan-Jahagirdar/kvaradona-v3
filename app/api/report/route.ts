...
export async function GET(){const locked=previewLockdown();if(locked)return locked;const c=await userClient();const {data:{user}}=await c.auth.getUser();if(!user)return NextResponse.json({error:'Sign in required'},{status:401});
