...
const Command=z.object({id:z.string().uuid(),revision:z.number().int().positive(),requestKey:z.string().uuid(),action:z.enum(['edit','recheck','defer','reject','research']),note:z.string().trim().min(1).max(3000),draft:Draft.nullable()});
