...
/** A company identity answer is structured minimal input, not free text parsed by the server. It is
 *  stored as an unverified hint; the stage checks it against that company's own published identity
 *  before anything is attributed to it. */
const IdentityAnswer=z.object({name:z.string().trim().min(1).max(200),
...
export async function POST(req:NextRequest){
 ...
 const {data,error}=await client.rpc('review_opportunity',{p_id:input.id,p_revision:input.revision,p_request:input.requestKey,p_action:input.action,p_note:input.note,p_draft:input.draft,
  ...(input.action==='identify_company'?{p_input:input.input}:{})});
 if(error)return NextResponse.json({error:error.message==='no_open_identity_question'?'This company has no open identity question to answer.':error.message==='identity_domain_invalid'?'Give the company website as a plain domain, for example example.com.':error.message==='identity_answer_required'?'A company name and its website are required.':error.message==='stale_version'?'This opportunity changed. Reload before reviewing.':error.message==='contact_refresh_reason_and_checked_packet_required'?'A checked research packet and a specific reason (20 characters) are required for a fresh buyer search.':error.message==='recovery_not_available'?'No automatic recovery is available for this state.':error.message==='recovery_in_progress'?'Recovery is already queued or running.':error.message==='ambiguous_operation_hold'?'A provider operation has an unresolved outcome. It must be reconciled before this stage can resume.':'Review could not be saved.'},{status:409});
