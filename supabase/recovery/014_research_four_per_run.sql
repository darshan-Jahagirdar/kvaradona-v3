-- Recovery: restore the three-research bound and the previous stored profile.
-- Preserves every opportunity, operation, reservation, job and reviewed draft.
...
update public.workflow_profiles
 set profile=jsonb_set(jsonb_set(profile,'{maxResearch}','3'::jsonb),'{maxCandidates}','5'::jsonb)
