-- Additive discovery history; original packets and evidence remain immutable.
alter table public.campaigns add column discovery_cursor integer not null default 0 check(discovery_cursor>=0);
alter table public.jobs add constraint jobs_org_campaign_id_key unique(organization_id,campaign_id,id);
create table public.discovery_observations (
 ...
 opportunity_id uuid not null, job_id uuid not null, source text not null, external_id text not null,
 event_key text not null, content_hash text not null, candidate jsonb not null,
 changed boolean not null default false, observed_at timestamptz not null default now(),
...
create index discovery_source_identity on public.discovery_observations(organization_id,campaign_id,source,external_id,observed_at desc);
create index discovery_opportunity on public.discovery_observations(opportunity_id,observed_at desc);
...
grant all on public.discovery_observations to service_role;
create policy discovery_member_read on public.discovery_observations for select to authenticated using
 (organization_id in(select organization_id from public.memberships where user_id=(select auth.uid())));

-- One active discovery job per campaign. The saved exclusion list cannot change on retry.
create function public.queue_discovery(p_campaign uuid) returns jsonb language plpgsql security invoker set search_path='' as $$
...
create or replace function public.tick_schedules() returns integer language plpgsql security invoker set search_path='' as $$
declare s public.schedules; result jsonb; n integer:=0;
begin
 for s in select q.* from public.schedules q join public.campaigns c on c.id=q.campaign_id where q.enabled and q.next_due<=now() and not c.paused for update of q skip locked loop
  result:=public.queue_discovery(s.campaign_id);
  update public.schedules set next_due=now()+make_interval(secs=>s.interval_seconds) where id=s.id;
  if (result->>'created')::boolean then n:=n+1; end if;
 end loop;return n;
end $$;
