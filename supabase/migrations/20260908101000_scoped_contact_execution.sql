-- Expiry exceptions require an explicit, time-limited grant for the exact S10 opportunity.
...
declare j public.jobs; o public.provider_operations; b public.budget; l public.provider_limits; total numeric; provider_total numeric; campaign_total numeric; opportunity_total numeric; consumed integer; scoped_apollo boolean;
begin
 ...
 scoped_apollo:=p_provider='apollo' and j.stage='S10' and coalesce((l.evidence::jsonb#>'{contactExecutionGrant,opportunityIds}') ? j.opportunity_id::text,false) and coalesce((l.evidence::jsonb#>>'{contactExecutionGrant,expiresAt}')::timestamptz>now(),false);
 if not found or not (l.probe_enabled or (l.authenticated and l.usable)) or l.verified_at is null or ((b.dollar_limits_enabled or p_provider not in ('openai','brave')) and not scoped_apollo and (l.expires_at<=now() or l.expires_at is null)) then raise exception 'provider_unverified'; end if;
