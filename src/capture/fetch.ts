...
export async function safeRead(input:string,limit=1000000){
 let current=input;
 for(let i=0;i<4;i++){
  ...
  try{
   const response=await request(url,{dispatcher:agent,signal:AbortSignal.timeout(12000),headers:{'user-agent':'KvaradonaResearch/0.1 (+evidence review)','accept':'text/html,text/plain,application/json;q=0.9'},headersTimeout:10000,bodyTimeout:10000});
   ...
   return {url:url.toString(),status:response.statusCode,contentType:String(response.headers['content-type']??''),text:Buffer.concat(chunks).toString('utf8'),bytes:Buffer.concat(chunks)};
