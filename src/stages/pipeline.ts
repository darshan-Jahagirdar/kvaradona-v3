...
export interface StageTools {
 ...
 contact:(host:string,role:string)=>Promise<Contact>;
...
function next(job:Job,stage:string,output:unknown){return {stage,business_key:`${job.opportunity_id}:${stage}:${job.input_version}`,input_hash:hash(output)};}
export async function runStage(store:Store,job:Job,tools:StageTools){
 ...
 if(job.stage==='S04'){
  ...
  else{p.contact=await tools.contact(p.research.accountHost,p.research.buyerRole);p.state=p.contact.state;nextStage='S11';}
  ...
  for(let attempt=0;attempt<2;attempt++){
   ...
   p.draftReview=await tools.ai.generate('A5',`draft_review_${attempt}`,Review,common,{...context(p),research:p.research,draft:p.draft,inputHash:hash(target),instruction:'Review the exact draft, including claims newly introduced by its writer. Check all material research claims and all assertions in the message. Echo inputHash exactly. Do not approve invented pain, intent, timings, credentials, proof, measurement or contact identity. List repairs in issues and verdicts.'});
