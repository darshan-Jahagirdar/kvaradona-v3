import {activeIntentTopics} from '../domain/intent-topics';
import {z} from 'zod';
import {required} from '../config/env';
import {Candidate} from '../contracts/pipeline';
import {ProviderCompany,ExploriumSearchDefinition,type DiscoveryGroup} from '../contracts/discovery';
import {hash,hostOf} from '../domain/policy';
import {exploriumSearchDefinition,exploriumIcp,exploriumPageSize,discoveryExcludedDomains} from '../domain/explorium-icp';
...
export function exploriumCandidates(body:unknown,observedAt=new Date().toISOString(),requestedTopics:readonly string[]=activeIntentTopics){
 ...
 const candidates:z.infer<typeof Candidate>[]=[];const seen=new Set<string>();let invalid=0,excluded=0;
 for(const raw of response.data){
  ...
  const signals=(r.business_intent_topics??[]).filter(s=>requestedTopics.includes(s.topic)&&s.score>60&&s.score<=100);
  if(!signals.length){invalid++;continue;}
  const companyKey=domain+':'+r.name.toLowerCase();if(seen.has(r.business_id)||seen.has(companyKey))continue;seen.add(r.business_id);seen.add(companyKey);
 ...
 return {candidates,providerResult:{returned:response.data.length,invalid,excluded,nextCursor:response.page?.next_cursor??null,topics:[...requestedTopics]}};
}
export function applyExploriumIntent(c:z.infer<typeof Candidate>,body:unknown,now=new Date(),requestedTopics:readonly string[]=activeIntentTopics){
 ...
 const pending=(reason:string)=>{c.providerCompany!.intent={...c.providerCompany!.intent,status:'unknown',reason};return c;};
 ...
 const matching=parsed.success?parsed.data.filter(t=>requestedTopics.includes(t.topic)&&t.composite_score>60):[];
...
export async function searchExploriumCompanies(operations:ContactOperations,key:string,group:DiscoveryGroup,request:typeof fetch=fetch,knownCompanies?:(candidates:z.infer<typeof Candidate>[])=>Promise<Set<string>>){
 if(group.nextCursor&&!group.searchDefinition)throw Error('intent_search_definition_required');
 const definition=ExploriumSearchDefinition.parse(group.searchDefinition??exploriumSearchDefinition()),requestedTopics=definition.filters.business_intent_topics.topics;
 if(requestedTopics.some(t=>!activeIntentTopics.includes(t)))throw Error('intent_topic_not_approved');
 const body={mode:definition.mode,page_size:definition.pageSize,filters:definition.filters,...(group.nextCursor?{next_cursor:group.nextCursor}:{})};
 // The measured intent-filtered request charged 2 credits/company. Reserve exactly that for the requested page.
 const result=exploriumCandidates(await callExplorium(operations,key,'businesses',body,2*definition.pageSize,request),new Date().toISOString(),requestedTopics);let enriched=0;
 if(result.providerResult.returned>definition.pageSize)throw Error('explorium_page_limit_exceeded');
 const known=await knownCompanies?.(result.candidates)??new Set<string>();let reused=0;
 for(const c of result.candidates){
  if(known.has(c.eventKey)){reused++;c.providerCompany!.intent.reason='Existing company retained; rediscovery does not restart research or buy duplicate intent enrichment.';continue;}
  if(c.providerCompany!.icp.status!=='match'||enriched>=definition.pageSize)continue;
  const detail=await callExplorium(operations,`${key}:intent:${c.providerCompany!.id}`,'businesses/bombora_intent/enrich',{business_ids:[c.providerCompany!.id],parameters:{topics:requestedTopics,min_score:61}},2,request);
  applyExploriumIntent(c,detail,new Date(),requestedTopics);enriched++;
 }
 return {...result,providerResult:{...result.providerResult,searchDefinition:definition,filters:body.filters,enriched,reused,httpStatus:200}};
}
