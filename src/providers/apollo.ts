...
export class ApolloContacts {
 ...
 async resolve(host:string,role:string,company:string):Promise<Contact>{
  ...
  let response;
  try{response=await this.call('contact_search','mixed_people/api_search',{'q_organization_domains_list[]':[host],'person_titles[]':contactTitles(role),include_similar_titles:'false',page:'1',per_page:'5'},0);}
  ...
  const candidates=parsed.data.people.filter(p=>p.organization&&normalize(p.organization.name)===normalize(company)&&roleScore(p.title??'',role)>0).sort((a,b)=>roleScore(b.title??'',role)-roleScore(a.title??'',role)).slice(0,5).map(p=>({providerId:p.id,displayName:[p.first_name,p.last_name_obfuscated].filter(Boolean).join(' ')||'Name withheld',role:p.title??role,company:p.organization!.name,refreshedAt:p.last_refreshed_at??null,emailAvailable:p.has_email===true,reason:'Provider-reported role/company match. Partial name and email availability do not establish an address or verified current employment.'}));
  ...
  const verified=person.id===selected.providerId&&employerHost===host&&normalize(person.organization?.name??'')===normalize(company)&&current&&roleScore(person.title??'',role)>=20;
