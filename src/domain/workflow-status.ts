...
export type WorkflowStatus={setup?:{ready:boolean;reason:string};companyCount?:number;run?:{id:string;created_at:string};worker:{online:boolean;seenAt:string|null};budget:{live_enabled?:boolean;spent_usd?:string;reserved_usd?:string;limit_usd?:string};jobs:{id:string;opportunity_id:string|null;stage:string;status:string;error:string|null}[];opportunities:{id:string;state:string;name:string}[]};
