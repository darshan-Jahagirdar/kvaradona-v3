...
// User approved 501-10,000 for this test. Only bands wholly inside that target are requested. The overlapping 201-500 band was dropped after two live runs:
...
export function exploriumIcp(row:{number_of_employees_range?:string|null;country_name?:string|null;business_description?:string|null;naics_description?:string|null}){
 ...
 else if(max<intentIcp.employeeRange.min||min>intentIcp.employeeRange.max){mismatch=true;reasons.push('Reported employee range is outside 501–10,000.');}
 else if(min<intentIcp.employeeRange.min||max>intentIcp.employeeRange.max)unknowns.push('The reported employee band crosses the ICP boundary; verify the exact count, without assuming a range proves the exact count.');
 else reasons.push('Reported employee band is within 501–10,000.');
...
export function exploriumSearchDefinition(){return {version:11,source:'explorium' as const,mode:'full' as const,pageSize:exploriumPageSize,filters:exploriumFilters()};}
